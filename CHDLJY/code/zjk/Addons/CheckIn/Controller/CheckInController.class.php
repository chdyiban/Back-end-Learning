    ...
    public function ranking()
    {
        ...
        if ($aOrder == 'today') {
            ...
            foreach ($user_list as $key => &$val) {
                ...
                $user = query_user(array('uid', 'name', 'total_check', 'con_check'), $val['uid']);
                $val = array_merge($val, $user);
            }
            unset($key, $val);

        } else {
            $user_list = $memberModel->field('uid,name,total_check,con_check')->page($aPage, $limit)->order($aOrder . ' desc,uid asc')->select();
            ...
            foreach($user_list as &$u){
                $temp_user=query_user(array('name'),$u['uid']);
        ...
        if (is_login()) {
            //获取用户信息
            $user_info = query_user(array('uid', 'name', 'space_url', 'avatar64', 'con_check', 'total_check'), is_login());
