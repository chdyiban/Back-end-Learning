...
class PublicController extends Controller
{
    ...
    private function getAtWhoUsers()
    {
        ...
        $users = array();
        foreach ($uids as $uid) {
            $user = query_user(array('name', 'id', 'avatar32'), $uid);
            $user['search_key'] = $user['name'] . D('PinYin')->Pinyin($user['name']);
            $users[] = $user;
        }
    ...
    public function assignUser(){
        ...
        $ids = explode(',',$aIds);
        foreach($ids as $v){
            $friends[] = query_user(array('avatar32', 'avatar64', 'space_url', 'name', 'uid', 'signature'), $v);
        }
