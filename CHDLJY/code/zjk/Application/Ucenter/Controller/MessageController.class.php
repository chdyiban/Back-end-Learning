...
class MessageController extends BaseController
{
    ...
    public function message($page = 1, $tab = 'unread')
    {
        $where = array();
        //从条件里面获取Tab
        $where = $this->getMapByTab($tab);
        $where['to_uid'] = is_login();

        //常规map为何不起作用？
        $messages = D('Message')->where($where)->order('create_time desc')->page($page, 10)->select();
        $totalCount = D('Message')->where($where)->order('create_time desc')->count(); //用于分页
...
//dump($messages);exit;
    ...
    public function talk($message_id = 0, $talk_id = 0)
    {
        ...
        foreach ($messages as &$mes) {
            $mes['user'] = query_user(array('avatar128', 'uid', 'username', 'name'), $mes['uid']);
        }
        unset($mes);
        $this->assign('messages', $messages);
