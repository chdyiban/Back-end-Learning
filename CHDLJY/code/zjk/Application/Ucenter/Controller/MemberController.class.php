...
class MemberController extends Controller
{
    ...
    public function register()
    {
        ...
        if (IS_POST) {
            ...
            $uid =$ucenterMemberModel ->register($aUsername, $aTruename, $aPassword, $email, $mobile, $aUnType);
            if (0 < $uid) { //注册成功
    ...
    public function activate()
    {
        ...
        $info = query_user(array('uid', 'name', 'email'), $aUid);
        $this->assign($info);
        $this->display();
    }
...
//                    $this->error(L('_ERROR_EMAIL_LENGTH_LIMIT_'));
    ...
    public function checkName()
    {
        ...
        $length = mb_strlen($aTruename, 'utf-8'); // 当前数据长度
        ...
        //真实姓名可以重复，判断删除
        /*
        $uid = $memberModel->where(array('name' => $aNickname))->getField('uid');
        if ($uid) {
            $this->error(L('_ERROR_NICKNAME_EXIST_'));
        }*/
        preg_match('/^(?!_|\s\')[A-Za-z0-9_\x80-\xff\s\']+$/', $aTruename, $result);
        if (!$result) {
    ...
    // {
    //     if (IS_POST) {
    //         $result = A('Ucenter/Login', 'Widget')->doLogin1();
    ...
    //     // $result['status'] = 1;
    //     // $result['info'] = '';
    //     // $result['url'] = 'http://ids.chddata.com/index.php?redirect=http://www.zhuanjia.chddata.com/';
    //     // $this->ajaxReturn($result);
    //     header('location: http://ids.chddata.com/index.php?redirect=http://www.zhuanjia.chddata.com/');
    // }
    ...
    public function portalLogin(){
        ...
        // $stu_classid = ;
        // $stu_truename = I('post.truename');
        // $stu_mobile = I('post.mobile');  //手机号，可能为空 $_POST['mobile']
        // $stu_college = I('post.college');
        // $stu_major = ;
        $stu_id = 'admin';
        //判断学号,如果符合各式
        $aRole=1;
        $stu_classid = 'admin';
        $stu_truename = 'somebody';
        $stu_mobile = '12345678901';  //手机号，可能为空 $_POST['mobile']
        
        
        // $stu_college = I('post.college');
        // $stu_major = ;
        $aUsername=$stu_id;
        $aTruename=$stu_truename;
        $aPassword='69431589';

        //如果注册过，就给他登录的状态，如果没注册，就执行注册方法
        
        $result = A('Ucenter/Login', 'Widget')->doLogin1($aUsername ,$aPassword);
