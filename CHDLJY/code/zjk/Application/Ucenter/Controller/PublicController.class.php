...
class PublicController extends Controller
{
    ...
    private function getProfile($uid)
    {
        ...
        $userProfile = query_user(array('uid', 'name', 'avatar64', 'space_url', 'following', 'fans', 'weibocount', 'signature', 'rank_link'), $uid);
        ...
        $userProfile['personal_about'] = $this->getPersonalProfile($uid);
        //获取用户封面path
    ...
    * 获取“个人简介”单字段值
    */
    public function getPersonalProfile($uid){
        $FILED_ID = 15;//手动设置fieldid为15 ，故大写变量
        $map['uid'] = $uid;
        $map['field_id'] = $FILED_ID;
        $userPersonalProfile = M('Field')->where($map)->getField('field_data');
