...
class ConfigController extends BaseController
{
    ...
    {
        $self = query_user(array('avatar128', 'name', 'space_url', 'space_link','score','title'));
...
1600:Lv7 '.L('_CHAIRMAN__').'
    ...
    public function index()
    {
        ...
        if (IS_POST) {
            ...
            clean_query_user_cache(get_uid(), array('name', 'sex', 'signature', 'email', 'pos_province', 'pos_city', 'pos_district', 'pos_community'));
            ...
            //调用API获取基本信息
            //TODO tox 获取省市区数据
            $user = query_user(array('name', 'signature', 'email', 'mobile', 'avatar128', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 'pos_community'), $aUid);
            //显示页面
     ...
     * @auth 陈一枭
     */
    private function checkNickname($name)
    {
        ...
        $match = preg_match('/^(?!_|\s\')[A-Za-z0-9_\x80-\xff\s\']+$/', $name);
        if (!$match) {
            $this->error(L('_ERROR_NICKNAME_LIMIT_').L('_PERIOD_'));
        }

        $map_nickname['name'] = $name;
