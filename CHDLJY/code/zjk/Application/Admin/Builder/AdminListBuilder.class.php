...
//            $this->setSearchPostUrl('');
    ...
    {
        return $this->key($name, $title, $subtitle, 'name');
    }
    ...
    public function display($solist = '')
    {
        ...
        $this->convertKey('uid', 'text', function ($value) {
            $value = query_user(array('name', 'uid', 'space_url'), $value);
            return "<a href='" . $value['space_url'] . "' target='_blank'>[{$value[uid]}]" . $value['name'] . '</a>';
        });

        //name转换成text
        $this->convertKey('name', 'text', function ($value) {
            $value = query_user(array('name', 'uid', 'space_url'), $value);
            return "<a href='" . $value['space_url'] . "' target='_blank'>[{$value[uid]}]" . $value['name'] . '</a>';
        });
