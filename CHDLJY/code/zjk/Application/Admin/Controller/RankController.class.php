...
class RankController extends AdminController
{
    ...
    public function index($page = 1, $r = 20)
    {
        ...
        foreach ($list as &$val) {
            $val['u_name'] = D('member')->where('uid=' . $val['uid'])->getField('name');
    ...
    public function userRankList($id = null, $page = 1)
    {
        ...
        $u_name = D('member')->where('uid=' . $id)->getField('name');
        $model = D('rank_user');
        ...
        if (IS_POST) {
            ...
            if ($is_Edit) {
                ...
                if (!$result) {
                    ...
                    //$u_name = D('member')->where('uid=' . $uid)->getField('name');
                    ...
                    $content1 = L('_TITLE_AWARD_ADMIN_PARAM_',array('name'=>$user['name'],'title'=>$rank['title'])) . $reason; //<img src="'.$logoUrl.'" title="'.$rank['title'].'" alt="'.$rank['title'].'">';
    ...
    public function rankVerify($page = 1)
    {
        ...
        foreach ($rankList as &$val) {
            ...
            $val['is_show'] = $val['is_show'] ? L('_SHOW_') : L('_NOT_SHOW_');
            //获取用户信息
            $u_user = D('member')->where('uid=' . $val['uid'])->getField('name');
            $val['u_name'] = $u_user;
        }
        unset($val);
        $builder = new AdminListBuilder();
    ...
    public function setVerifyStatus($ids, $status)
    {
        ...
        if ($status == 1) {
            foreach ($ids as $val) {
                ...
                $user = query_user(array('name', 'space_link'), $rank_user['uid']);

                $content1 = L('_RECEPTION_PASSED_BY_ADMIN_PARAM_',array('name'=>$user['name'],'title'=>$rank['title'])) . $rank_user['reason'];
