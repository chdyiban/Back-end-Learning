...
class UserController extends AdminController
{
    ...
    public function expandinfo_select($page = 1, $r = 20)
    {
        ...
        $fields_list = array_slice($fields_list, 0, 15);//取出前8条，用户扩展资料默认显示8条
        foreach ($list as &$tkl) {
            $tkl['id'] = $tkl['uid'];
        ...
        $builder->setSearchPostUrl(U('Admin/User/expandinfo_select'))->search(L('_SEARCH_'), 'name', 'text', L('_PLACEHOLDER_NICKNAME_ID_'));
        $builder->keyId()->keyLink('name', L('_NICKNAME_'), 'User/expandinfo_details?uid=###');
    ...
    public function expandinfo_details($uid = 0)
    {
        if (IS_POST) {
            ...
            $builder->keyId()->keyReadOnly('username', L('_USER_NAME_'))->keyReadOnly('name', L('_NICKNAME_'));
            $field_key = array('id', 'username', 'name');
            foreach ($fields_list as $vt) {
    ...
    {
        $name = M('Member')->getFieldByUid(UID, 'name');
    ...
    public function submitNickname()
    {
        ...
        empty($name) && $this->error(L('_PLEASE_ENTER_A_NICKNAME_'));
        ...
        $Member = D('Member');
        $data = $Member->create(array('name' => $name));
        if (!$data) {
