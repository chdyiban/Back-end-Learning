...
class MessageController extends AdminController
{
    ...
    public function userList($page=1,$r=20)
    {
        ...
        if (empty($aSearch1) && empty($aSearch2)) {
            ...
            $user = D('member')->where($map)->page($page, $r)->field('uid,name')->select();
            foreach ($user as &$v) {
                $v['id'] = $v['uid'];
            }
            unset($v);
        ...
        $builder->keyText('uid', '用户ID')->keyText('name', L('_"name"_'));
    ...
    private function getUids_sc($search_nn = "", $search_id = 0)
    {
        $uids = array();
        if (!empty($search_nn)) {
            $users = D('member')->where(array('name' => $search_nn))->field('uid')->select();
    ...
    public function sendMessage()
    {
        ...
        if (IS_POST) {
            ...
            if (empty($aUids)) {
                ...
                $uids = implode(',',$aUids);
                $users = D('Member')->where(array('uid'=>array('in',$aUids)))->field('uid,name')->select();
