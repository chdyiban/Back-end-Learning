...
class MemberModel extends Model
{
    ...
    protected $_validate = array(
        ...
        /* 验证昵称 */
        array('name', 'checkName', -33, self::EXISTS_VALIDATE, 'callback'), //昵称长度不合法
        array('name', 'checkDenyName', -31, self::EXISTS_VALIDATE, 'callback'), //昵称禁止注册
        array('name', 'checkNameLength', -32, self::EXISTS_VALIDATE, 'callback'),
        //array('name', '', -30, self::EXISTS_VALIDATE, 'unique'), //昵称被占用

    );

    protected $insertField = 'name,sex,birthday,qq,signature'; //新增数据时允许操作的字段
    protected $updateField = 'name,sex,birthday,qq,signature,last_login_ip,login,update_time,last_login_role,show_role,status,tox_money,score,pos_province,pos_city,pos_district,pos_community'; //编辑数据时允许操作的字段
    ...
    protected function checkName($name)
    {
        ...
        preg_match('/^(?!_|\s\')[A-Za-z0-9_\x80-\xff\s\']+$/', $name, $result);
     ...
     */
    protected function checkNameLength($name)
    {
    ...
    {
        /* 在当前应用中注册用户 */
        if ($user = $this->create(array('name' => $name, 'status' => 1))) {
            $uid = $this->add($user);
            if (!$uid) {
    ...
    {
        //去除特殊字符。
        $data['name'] = preg_replace('/[^A-Za-z0-9_\x80-\xff\s\']/', '', $info['nick']);
        // 截取字数
        $data['name'] = mb_substr($data['name'], 0, 32, 'utf-8');
        // 为空则随机生成
        if (empty($data['name'])) {
            $data['name'] = $this->rand_name();
        } else {
            if ($this->where(array('name' => $data['name']))->select()) {
                $data['name'] .= '_' . $uid;
            }
        }
        $data['sex'] = $info['sex'];
        $data = $this->validate(
            ...
            /* 验证姓名 */
            array('name', 'checkDenyName', -31, self::EXISTS_VALIDATE, 'callback'), //昵称禁止注册
            array('name', 'checkName', -32, self::EXISTS_VALIDATE, 'callback'))->create($data);
        $data['uid'] = $uid;
        $res = $this->add($data);
        return $res;
    }
    ...
    {
        $name = create_rand(4);
        if ($this->where(array('name' => $name))->select()) {
            $this->rand_nickname();
        } else {
            return $name;
        }
