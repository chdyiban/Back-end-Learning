...
class FollowModel extends Model
{
    ...
    public function follow($uid)
    {
         ...
         */
        $user = query_user(array('id', 'name', 'space_url'));
        $this->S($follow['who_follow'], $follow['follow_who'], null);
        D('Message')->sendMessage($uid, L('_FANS_NUMBER_INCREASED_'), $user['name'] . L('_CONCERN_YOU_WITH_PERIOD_'), 'Ucenter/Index/index', array('uid' => is_login()));
    ...
    public function unfollow($uid)
    {
        ...
        D('Message')->sendMessage($uid, L('_NUMBER_OF_FANS_'), $user['name'] . L('_CANCEL_YOUR_ATTENTION_WITH_PERIOD_'), 'Ucenter/Index/index', array('uid' => is_login()));
    ...
    public function addFollow($who_follow, $follow_who, $invite = 0)
    {
         ...
         * @param int $type 消息类型，0系统，1用户，2应用
         */
        $user = query_user(array('id', 'name', 'space_url'), $who_follow);
        if ($invite) {
            if ($who_follow < $follow_who) {
                $content = L('_INVITED_') . $user['name'] . L('_CONCERN_YOU_WITH_PERIOD_');
            } else {
                $content = L('_YOURE_INVITING_THE_USER_') . $user['name'] . L('_CONCERN_YOU_WITH_PERIOD_');
            }
        } else {
            if ($who_follow < $follow_who) {
                $content = L('_SYSTEM_RECOMMENDED_USERS_') . $user['name'] . L('_CONCERN_YOU_WITH_PERIOD_');
            } else {
                $content = L('_NEW_USER_') . $user['name'] . L('_CONCERN_YOU_WITH_PERIOD_');
            }
        }
