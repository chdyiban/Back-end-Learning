...
class TalkModel extends Model
{
    ...
    public function getLastMessage($talk_id)
    {
        ...
        $last_message['user'] = query_user(array('name', 'space_url', 'id'), $last_message['uid']);
    ...
    public function createTalk($members)
    {
        ...
        if (count($orin_member) == 1) {
            $user_one = query_user(array('name'), $orin_member[0]);
            $user_two = query_user(array('name'));
            $talk['title'] = $user_two['name'] . ' 和 ' . $user_one['name'] . L('_CHAT_');
        }
    ...
    private function getFirstUserAndLastMessage($li)
    {
        ...
        foreach ($uids as $uid) {
            if ($uid != is_login()) {
                $li['first_user'] = query_user(array('avatar64', 'name'), $uid);
                break;
            }
    ...
    public function getFirstOtherUser($members)
    {
        ...
        foreach ($members as &$mem) {
            if ($mem != is_login() && $has_got_ico == false) {
                $ico_user = query_user(array('avatar64', 'name'), $mem);
                $has_got_ico = true;
            }
        }
        unset($mem);
