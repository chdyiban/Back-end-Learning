...
class UserModel
{
    private $table_fields = array(
        //member
        'uid', 'name', 'sex', 'birthday', 'qq', 'show_role','signature', 'pos_province', 'pos_city', 'pos_district', 'pos_community', 'score1', 'score2', 'score3', 'score4',
        //ucmember
    ...
    private function getFields($pFields)
    {
        //默认赋值
        if ($pFields === null) {
            return array('name', 'space_url', 'space_mob_url', 'avatar32', 'avatar64', 'avatar128', 'uid');
        }
    ...
    function query_user($pFields = null, $uid = 0)
    {
        ...
        //必须强制处理昵称备注
        if (in_array('name', (array)$pFields))
...
//获取个人中心地址
        ...
        if (array_intersect(array('space_url', 'space_link', 'space_mob_url'), $fields)) {
            ...
            $urls['space_link'] = '<a ucard="' . $uid . '" target="_blank" href="' . U('Ucenter/Index/index', array('uid' => $uid)) . '">' . $result['name'] . '</a>';
...
//读取用户名拼音
        if (in_array('pinyin', $fields)) {
            ...
            $result['pinyin'] = D('Pinyin')->pinYin($result['name']);
            return $result;
        }
        return $result;
    }
    ...
    public function getNickname($fields, $ucenterResult)
    {
        if (in_array('name', $fields)) {
            $ucenterResult['name'] = text($ucenterResult['name']);
