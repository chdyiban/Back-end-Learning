...
function parse_at_users($content, $disabel_hight = false)
{
    ...
    foreach ($at_users as $e) {
        ...
        if ($user) {
            $query_user = query_user(array('space_url', 'avatar32', 'name'), $user['uid']);
            if (modC('HIGH_LIGHT_AT', 1, 'Weibo') && !$disabel_hight) {
                $content = str_replace("[at:$e]", " <a class='user-at hl ' ucard=\"$user[uid]\" href=\"$query_user[space_url]\"><img src=\"$query_user[avatar32]\">@$query_user[name] </a> ", $content);
            } else {
                $content = str_replace("[at:$e]", " <a ucard=\"$user[uid]\" href=\"$query_user[space_url]\">@$query_user[name] </a> ", $content);
            }
...
function parse_at_app_users($content, $disabel_hight = false)
{
    ...
    foreach ($at_users as $e) {
        $user = D('Member')->where(array('uid' => $e))->find();
        if ($user) {
            $query_user = query_user(array('avatar32', 'name'), $user['uid']);
            if (modC('HIGH_LIGHT_AT', 1, 'Weibo') && !$disabel_hight) {
                $content = str_replace("[at:$e]", " <a class='user-at' data-user-id=\"$user[uid]\">@$query_user[name] </a> ", $content);
            } else {
                $content = str_replace("[at:$e]", " <a data-user-id=\"$user[uid]\">@$query_user[name] </a> ", $content);
            }
        }
    }
    //返回替换的文本
...
//    $users[$k]=M('Member')->where(array('name'=>$c[$k]))->find();
//}
    //返回用户名列表
